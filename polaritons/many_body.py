...
def effective_mass(model: DispersionModel, eta: float, k_grid: np.ndarray,
    ...
    k_grid         : dense k array (m^-1) starting near 0
    disorder_tuned : cavity tuning flag passed to E_LP
    """
    E_vals = model.E_LP(k_grid, eta, disorder_tuned=disorder_tuned)
    d2E    = np.gradient(np.gradient(np.real(E_vals), k_grid), k_grid)
    return model.p.hbar**2 / d2E[0]
...
def polariton_interaction_strength(
    ...
    bubble = Pi0(model, eta, beta=beta, L_terms=L_terms,
